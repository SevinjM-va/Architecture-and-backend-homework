...
def create_app():
    ...
    def sglistsingle():
            ...
            if own == "Y":
                ...
                if lowest_comp_price != "NO COMP":
                    ...
                    # positive difference == lower bprice
                        ...
                        # If we are at FV but are no longer the lowest,
                        # set the price to BFLOOR.
                        # BFLOOR compares with the 2nd lowest price and subtracts 1
                        # to avoid strange pricing that pushes our price down.

                        return fv, "BFLOOR"
                    elif (difference > 0) & (size > ownsize):
                        return round(suggestedbprice, 2), "MRKDWN"
                    # Negative difference = increase bprice,
                    # but only if the difference is more than 5 dollars.
                    # need to find the 2nd lowest comp price that is "N" then push that.
                    # This should be separated into another column
        ...
        if ownsizedf.empty:
            ...
            #     min_fv=("FACE_VALUE", "min"),
            #     mean_fv=("FACE_VALUE", "mean"),
            # )
            ...
            # Find the markup 2nd lowest price only if
            # there is more than one count and no changes.
            ...
            if "section" not in newdfcols:
                ...
                pass
            # newdf.drop(
            #     ["row", "section", "SIZE", "bprice", "2ND_LOWEST"],
            #     axis=1,
            #     inplace=True,
            # )
            colstokeepfinal = [
    ...
    def trtmlbcounts():
        ...
        def bginternal(x):
            ...
            if sh == "Y":
                intnotestr = (
                    "BG:%s -vivid -ticketevolution -tickpick "
                    "-ticketnetwork -ticketmaster" % (intnoteval)
                )
            else:
                intnotestr = (
                    "BG:%s -vivid -ticketevolution -tickpick -ticketnetwork "
                    "-stubhub -gametime -ticketmaster" % (intnoteval)
                )
            if unbroadcast != "Unbroadcast":
                return intnotestr
            else:
